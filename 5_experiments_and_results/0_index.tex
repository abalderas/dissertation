...
%En los antecedentes se trabaja en la extracción de indicadores sobre un wiki de MediaWiki, mientras que después se presentan dos DSLs para la extracción de indicadores. El primero de ellos a partir de los registros de un LMS (Moodle), mientras que el segundo lo hace a partir de los registros de un mundo virtual basado en OpenSim.
...
\end{figure}
			...
			\subparagraph*{Comunicación y aplicación del conocimiento (COM y KNO)}
...
\begin{table}
  ...
  \begin{tabular}{| L{1.5cm} | L{5.5cm} | L{5.5cm} |}
    \hline 
    \multirow{2}{*}{COMP.}  & INDICADORES  & INDICADORES  \\
...
%		\end{itemize} 
		...
		\subsection{EvalCourse y SASQL} \label{subcha:evc}
...
\end{minted}
			...
			Para evaluar las competencias genéricas el docente deberá definir los indicadores que serán extraídos de la actividad de cada estudiante en el LMS. Se ilustra el método a partir de un ejemplo de uso de EvalCourse y su ejecución con los foros del LMS. Los foros suelen ser una de las herramientas incorporadas por los LMS para la interacción entre los estudiantes. Es evidente que la comunicación oral es una manera muy rica de comunicarse, que proporciona múltiples signos no verbales como las expresiones faciales o el tono de voz. En contraste, las comunicaciones escritas proporcionan otras ventajas. Una de las más importantes para la educación es que el estudiante dispone de un tiempo para la reflexión. Por esta razón, podría preferirse la comunicación escrita a la oral cuando se busca un aprendizaje cognitivo~\cite{garrison1999critical}.
...
\end{figure}
			...
			En este ejemplo, el docente pretende obtener indicadores del uso del foro para evaluar la competencia genérica de las habilidades interpersonales (INT). Durante el curso, los estudiantes interactuarán en el foro conforme a las instrucciones proporcionadas por el docente. Cuando lo desee, el docente podrá utilizar EvalCourse para obtener los indicadores.
...
%\begin{lstlisting}[caption=Consulta para obtener la participación en el foro en un período concreto de tiempo ,label=code:sasqlejemplo1,numbers=left, captionpos=b, morekeywords={Evidence,get, students, show, milestones, participation, access, in, assignment, forum, campus, workshop, interaction, between, and}]
...
\end{table}
			...
			A la vista de los resultados, y en base a la hipótesis inicial, se podría decir que todos los estudiantes menos el 3 (\emph{student3}) habrían desempeñado correctamente la competencia. Sin embargo, el docente considera que esta primera aproximación es un poco pobre y decide completar su hipótesis de la siguiente manera: \emph{se considerará que un estudiante ha desempeñado satisfactoriamente la competencia genérica de las INT si ha escrito al menos dos mensajes en el foro y ha interactuado con más de un compañero en la semana}. Para ello escribe la consulta SASQL interacciones foro.

%\begin{lstlisting}[caption=Consulta para obtener la interacción en el foro en un período de tiempo determinado, label=code:sasqlejemplo2,numbers=left, captionpos=b, morekeywords={Evidence,get, students, show, milestones, participation, access, in, assignment, forum, campus, workshop, interaction, between, and}]
...
\begin{figure}
	...
	\caption{Interacción en el foro en un período de tiempo.}
...
\end{itemize}
			...
			\subparagraph*{Capacidad de aprender y mantenerse al día con el aprendizaje (LLL)}
...
% TMW, \emph{capacidad crítica}, la \emph{comunicación}, la \emph{aplicación del conocimiento}, la \emph{capacidad para mantener la calidad del trabajo producido} y la \emph{capacidad crítica de sus estudiantes}.
...
%l pensamiento crítico desempeñado por los estudiantes en los talleres.
...
\subparagraph*{Planificación y gestión del tiempo (PLA).}
